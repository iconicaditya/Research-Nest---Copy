...
export default function Team() {
  ...
  const pi = teamMembers.filter((m: any) => m.role.includes("Principal Investigator"));
  const others = teamMembers.filter((m: any) => !m.role.includes("Principal Investigator"));
          ...
          {pi.length > 0 && (
            <>
              ...
              <div className="grid md:grid-cols-2 gap-8 mb-16">
                {pi.map((member: any) => (
                  <div key={member.id} className="flex flex-col md:flex-row gap-8 bg-card rounded-xl p-6 border shadow-sm" data-testid={`team-member-${member.id}`}>
            ...
            <>
              ...
              <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
                {others.map((member: any) => (
                  <Card key={member.id} className="text-center hover:shadow-lg transition-shadow" data-testid={`team-member-${member.id}`}>
